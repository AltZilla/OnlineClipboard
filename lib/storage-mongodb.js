...
export async function checkClipboardExists(id) {
    try {
        await connectDB();
        ...
        console.error('Error checking clipboard existence:', error);
        return false;
    }
}
...
export async function getFileBuffer(id, filename) {
    try {
        await connectDB();
        const clipboard = await Clipboard.findOne(
            { id, 'files.filename': filename, expiresAt: { $gt: new Date() } },
            { 'files.$': 1 }
        );
...
export async function getAllClipboards(limit = 50, publicOnly = false) {
    try {
        ...
        query.expiresAt = { $gt: new Date() }; // Only show actively non-expired boards
